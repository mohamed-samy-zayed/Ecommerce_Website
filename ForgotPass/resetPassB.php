<?php
/* I Get Token From Post Form Not From The Link Param I Store This Token In The Form In The First Click On This Link*/
require '../sec.php';
session_start();
    ...
    if (isset($_POST['np']) && !empty($_POST['np']) && isset($_POST['c_np']) && !empty($_POST['c_np'])) {
        ...
        if ($new_pass == $conf_new_pass) {
            ...
            if ($con->connect_error) {
                ...
                $stmt = $con->prepare("SELECT * FROM user WHERE email = ? AND LOWER(reset_token) = LOWER(?)");
