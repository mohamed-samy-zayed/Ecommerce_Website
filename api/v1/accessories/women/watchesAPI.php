...
else
{
    ...
    {
        $stmt = $con->prepare("SELECT * FROM women_watches WHERE type = ?");
    ...
    elseif($filter == "0-100")
    {
        $min = 0;
        $max = 100;
        $stmt = $con->prepare("SELECT * FROM women_watches WHERE price BETWEEN ? AND ? AND type = ? ");
    ...
    {
        $c="black";
        $stmt = $con->prepare("SELECT * FROM women_watches WHERE color = ? AND type = ?");
