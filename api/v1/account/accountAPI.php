...
session_start();
if(isset($_SESSION['id']) && !empty($_SESSION['id']) && isset($_SESSION['api_token']) && !empty($_SESSION['api_token']))
{
    ...
    // Check IF Api Token Is Allready Exsits IN User Database OR Not
    /* $query_Check_API = "SELECT api_token FROM user WHERE api_token = ? AND id = ?"; */
    $stmt_Check_API = $con->prepare("SELECT * FROM user WHERE id = ? AND api_token= ? ");
    ...
    {
        $stmt = $con->prepare("SELECT id,name,email,profile_img_url FROM user WHERE id = ? ");
