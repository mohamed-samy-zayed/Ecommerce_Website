...
{
    $stmt = $con->prepare("SELECT * FROM table_watch");
    $stmt->execute();
...
elseif($filter == "0-100")
{
    $min = 0;
    $max = 100;
    $stmt = $con->prepare("SELECT * FROM table_watch WHERE price BETWEEN ? AND ? ");
...
{
    $c="black";
    $stmt = $con->prepare("SELECT * FROM table_watch WHERE color = ? ");
    $stmt->bind_param("s",$c);
    $stmt->execute();
    $result = $stmt->get_result();
    if($result->num_rows > 0)
