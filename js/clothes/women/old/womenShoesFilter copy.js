// Get all Products
fetch('http://localhost/all/project/ecommerce/api/v1/clothes/women/shoesAPI.php')
...
checkboxes.forEach((checkbox) => {
    ...
    if (checkbox.checked) {
      ...
      // Log or store the selected price range
        ...
        var url = `http://localhost/all/project/ecommerce/api/v1/clothes/women/shoesAPI.php?filter=${filter}`;
        fetch(url)
        .then(response => response.json())
        .then(data => {
            // Update the frontend with the product data
...
ref.textContent = document.getElementById('prev_ref').value;
ref.setAttribute("name","p_ref");
