...
setTimeout(function () {// Show All Products 
    fetch('http://localhost/all/project/ecommerce/api/v1/furniture/electric/electricAPI.php')
    ...
    if (checkbox.checked) {
      ...
      // Log or store the selected price range
        ...
        setTimeout(function () {
                ...
                var url = `http://localhost/all/project/ecommerce/api/v1/furniture/electric/electricAPI.php?filter=${filter}`;
                fetch(url)
                .then(response => response.json())
                .then(data => {
            // Hide loading indicator for filtered products
